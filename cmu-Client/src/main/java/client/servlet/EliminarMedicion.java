 ...
 * @author oscar
 */
@WebServlet(name = "EliminarMedicion", urlPatterns = {"/eliminarMedicion"})
public class EliminarMedicion extends HttpServlet {

    @WebServiceRef(wsdlLocation = "WEB-INF/wsdl/155.210.71.106_8080/CMU_server/CMUService.wsdl")
    private CMUService_Service service;
    private Medicion medicionEliminar;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try { // Call Web Service Operation
            ...
            medicionEliminar = port.getMedicionID(id);
            // TODO process result here

            if (medicionEliminar != null) {
                port.removeMedicion(medicionEliminar);
                response.sendRedirect(response.encodeURL("pantallatemperaturas.jsp"));
            }
        } catch (Exception ex) {
